...
| THS-F011 | `test_grant_manager.py`; authority/API durable expiry/window/digest/revoke/suspension tests; owner status/snapshot first-observer and restart proof; re-arm leaves grants suspended | Pass |
...
| THS-F014 | `test_grant_store.py`, `test_grant_authority.py`: restart persistence, owner-projection expiry persistence, restrictive recovery, corruption and pending semantic tamper rejection, prior/target witnesses, stale/concurrent instances, authorization lease, zero-active ESTOP revision | Pass, 18 + 24 focused tests |
...
| Automated suite | Wave 7 Python compile/tests/release scan plus console unit/interaction tests, TypeScript check, production build, and accessibility scan | Pass locally, 454 Python and 18 console tests |
...
| Python 3.10/3.12 CI | GitHub Actions is required on every pushed PR head; the prior `9ee289a` head passed, while this selected local review fix has not been pushed | Pending for any authorized updated PR head; GitHub status is authoritative |
