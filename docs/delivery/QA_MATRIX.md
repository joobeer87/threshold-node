...
| Python 3.10/3.12 CI | Prior-wave CI plus Wave 7 local backend proof | Pending PR #7 publication/check run |
...
it does not make materialized output live. Delivery remains at `warn` because human visual
review, a live synthetic model evaluation, real-dwelling review, and later hardware/adapter
proof are incomplete. Model/spatial inference, automatic materialization, command relay,
physical hardware, deployment, and full-demo claims stay blocked.
